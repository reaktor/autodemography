...
autot<-tbl(trafi.db,"henkiloauto_viimeisin") %>% 
  select(merkkiSelvakielinen,mallimerkinta,kaupallinenNimi,ajoneuvoryhma,korityyppi,data,N.id) %>% 
  collect(n=Inf)
...
write.table(merkit.stat,file="merkitmap.csv",quote=FALSE,sep="\t",row.names=FALSE,

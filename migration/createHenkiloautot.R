...
# yksi yhtä data id:tä (4.02, 4.03, ...) kohti. Näille annetaan pseudo.id
...
h <- mutate(h,
            ...
            record.id = ifelse(stabile.alue, 
                               paste("1", str_pad(alue, 3, side = "left", pad = 0), str_pad(combo, 8, side = "left", pad = "0"), sep = ""), 
                               record.id),
...
if (db_has_table(trafi.db$con, "henkiloautot")) 
  db_drop_table(trafi.db$con, "henkiloautot") 

as.data.frame(henkiloautot) %>% 
  db_insert_into(trafi.db$con, "henkiloautot", .)
...
if (db_has_table(trafi.db$con,"henkiloauto_historia")) 
  db_drop_table(trafi.db$con, "henkiloauto_historia")

as.data.frame(henkiloauto.historia) %>% 
  db_insert_into(trafi.db$con, "henkiloauto_historia",.)
...
if (db_has_table(trafi.db$con,"henkiloauto_uniqcombos")) 
  db_drop_table(trafi.db$con, "henkiloauto_uniqcombos")

as.data.frame(henkiloauto.uniikit) %>% 
  db_insert_into(trafi.db$con, "henkiloauto_uniqcombos",.)
...
write.table(henkiloauto.historia, 
            file = full.path("henkiloautot.txt"), 
            quote = FALSE,
            sep = "\t", 
            na = "", 
            dec = ".", 
...
write.table(henkiloauto.historia, 
            file = full.path("henkiloauto_historia.txt"), 
            quote = FALSE,
            sep = "\t", 
            na = "", 
            dec = ".", 
...
write.table(henkiloauto.uniikit, 
            file = full.path("henkiloauto_uniqcombos.txt"),
            quote = FALSE,
            sep="\t", 
            na="", 
            dec=".", 
